...
#define RVAL 16   // set this value as 2^M where M is the parameter of SRRIP/DRRIP
...
struct cache_blk_t * take_miss_action(struct cache_set_t* set) {

      struct cache_blk_t    *blk;
      unsigned int largest;
      struct cache_blk_t    *repl;
      
      largest = set->way_head->rrpv;
      repl = set->way_head;
      for (blk=set->way_head;
           blk;
           blk=blk->way_next)
        {
           if(blk->rrpv == RVAL-1) {
           ...
           if(blk->way_next == NULL ) { // (2^M)-1 not found after reaching end, so increment all rrpvs
              for (blk=set->way_head;
                   blk;
                   blk=blk->way_next) {
                   blk->rrpv++;
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  blk->tag = 0;
	  blk->ready = 0;
          blk->rrpv = RVAL-1; // SRRIP / DRRIP assume M = 2 SRS TODO code to accept from command line
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case SRRIP:
    repl = take_miss_action(&cp->sets[set]);
    break;

  case DRRIP:
    if(cp->sets[set].policy_counter <= 1 ) { // we used SRRIP which resulted in a miss 
                                               // so update policy_counter accordingly
      if(cp->sets[set].policy_counter < 3) {   // move towards using Bimodal
        cp->sets[set].policy_counter++;
      }
    } else if(cp->sets[set].policy_counter > 1 ) { // used Bimodal last, move towards SRRIP
      cp->sets[set].policy_counter--;
    }
    repl = take_miss_action(&cp->sets[set]);   // repl action is same as SRRIP
    break;
  ...
  if(cp->policy == SRRIP ) 
    ...
    if(cp->sets[set].policy_counter <= 1 ){ // SRRIP
      repl->rrpv = RVAL-2;
    } else { // Bimodal
 ...
 cache_hit: /* slow hit handler */
  ...
  if (blk->way_prev && cp->policy == LRU)
	...
	blk->rrpv--;
      ...
      if(cp->sets[set].policy_counter == 2 ) { 
          cp->sets[set].policy_counter++;
      } else if(cp->sets[set].policy_counter == 1 ) {
          cp->sets[set].policy_counter = 0;
      }
